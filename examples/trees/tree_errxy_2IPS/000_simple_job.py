...
for i in range(cfg['number_of_iterations']):
    ...
    if 'sigmaz' in dict_shift:
        [sol_LS[i],f_sol[i],Jac_sol[i],timing_LS,nfev_LS,delta_sigmaz[f'par_it{i}']] = inv_g.inv_gauss_xy_randerr_2(epsx1,epsy1,dict_shift)
    else:
        [sol_LS[i],f_sol[i],Jac_sol[i],timing_LS,nfev_LS] = inv_g.inv_gauss_xy_randerr_2(epsx1,epsy1,dict_shift)
