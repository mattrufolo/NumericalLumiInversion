...
for i in range(1):
    ...
    with open(f"output_zoom.txt","w") as h,contextlib.redirect_stdout(h):
        [sol_LS[i+1],f_sol[i+1],Jac_sol[i+1],timing_LS,nfev_LS] = inv_g.inv_gauss_xy(eps1,eps2,dict_shift,verbose=2)
...
with open('output_zoom_epsxy.txt') as f:
    contents = f.read()
...
def penalty_square(xy):
    ...
    for i in range(int(len(constants))-1):
            penalty += (L_over_eps_xy(xy[0],xy[1],paramx[f'par'][keys[i]])-constants[(i+1)])**2
