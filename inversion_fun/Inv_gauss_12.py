...
#     else:
    return [roots.x,roots.fun, roots.jac, time_LS, roots.nfev]
def inv_gauss_single_par(epsx1,epsy1,epsx2,epsy2,dict_shift,nfev = 3000, verbose = 0):
    '''
    Inversion the luminosity function in order to obtain the emittances, in the case in which the model
    is coherent and also the parameters, shifting the parameters in the dict_shift
    but once a time, can be generalized to multiple shift at the same time wrt to the reference configuration.
    ...
    for i in range(len(dict_shift)):
        ...
        for j in range(len(dict_shift[param])):
            ...
            #     shift_par  = abs(shift_par)
            # print(shift_par)
            delta_par[f'{dict_shift[param][j]}{param}'] = copy.copy(parameters_sym_1)
            delta_par[f'{dict_shift[param][j]}{param}'][dict_par[param]]+= shift_par
    ...
    for i in range(len(dict_shift)):
        param = list(dict_shift.keys())[i]
        for j in range(len(dict_shift[param])):
            constants = np.concatenate([constants,[L_over_parameters_sym(epsx1,epsy1,epsx2,epsy2, par = delta_par[f'{dict_shift[param][j]}{param}'])]])
    print(constants)
    ...
    def func_to_zero(x):
        ...
        for i in range(len(dict_shift)):
            param = list(dict_shift.keys())[i]
            for j in range(len(dict_shift[param])):
                output = np.concatenate([output,[L_over_parameters_sym(x[0],x[1],x[2],x[3], par = delta_par[f'{dict_shift[param][j]}{param}'])-\
                    constants[i*(len(dict_shift[param]))+1+j]]])
        return output
    if verbose == 2:
        def log_iteration(x):
...
def inv_gauss_single_par_err(epsx1,epsy1,epsx2,epsy2,noise_px1,dict_shift, nfev = 5000):
    '''
    Inversion the luminosity function in order to obtain the emittances, in the case in which the model
    is coherent with the problem but the parameters are questioned for some intrinsic errors, shifting the parameters in the dict_shift
    but once a time, can be generalized to multiple shift at the same time wrt to the reference configuration.
    ...
    parameters_sym_err = [f,nb,N,N,energy_tot,energy_tot,mu0x,mu0y,-mu0x,-mu0y,
                            px+noise_px1,py,-px,-py,
                            alphax,alphay,alphax,alphay,
                            sigmaz,sigmaz,
                            betax,betay,betax+1e-4,betay+1e-4,
                            deltap_p0,deltap_p0,dmu0x,dmu0y,-dmu0x,-dmu0y,dpx,dpy,-dpx,-dpy]
    ...
    for i in range(len(dict_shift)):
        ...
        print(dict_shift[param])
        for j in range(len(dict_shift[param])):
            print(dict_shift[param][j])
            shift_par= fsolve(lambda x:percent_sym_short(epsx1,epsy1,epsx2,epsy2,x,param,dict_shift[param][j]), x0 = 0)[0]
            print(shift_par)
            delta_par[f'{dict_shift[param][j]}{param}'] = copy.copy(parameters_sym)
            delta_par_err[f'{dict_shift[param][j]}{param}'] = copy.copy(parameters_sym_err)
            delta_par[f'{dict_shift[param][j]}{param}'][dict_par[param]]+= shift_par
            delta_par_err[f'{dict_shift[param][j]}{param}'][dict_par[param]]+= shift_par
        
            if param in ['betax','betay','alphax','alphay']:
                delta_par[f'{dict_shift[param][j]}{param}'][dict_par[param]+2]+= shift_par
                delta_par_err[f'{dict_shift[param][j]}{param}'][dict_par[param]+2]+= shift_par
            if param in ['sigmaz']:
                delta_par[f'{dict_shift[param][j]}{param}'][dict_par[param]+1]+= shift_par
                delta_par_err[f'{dict_shift[param][j]}{param}'][dict_par[param]+1]+= shift_par
    
    constants = [L_over_parameters_sym(epsx1,epsy1,epsx2,epsy2, par = parameters_sym_err)]
    for i in range(len(dict_shift)):
        param = list(dict_shift.keys())[i]
        for j in range(len(dict_shift[param])):
            constants = np.concatenate([constants,[L_over_parameters_sym(epsx1,epsy1,epsx2,epsy2, par = delta_par_err[f'{dict_shift[param][j]}{param}'])]])

    def func_to_zero(x):
        output = [L_over_parameters_sym_noise(x[0],x[1],x[2],x[3],x[4])-constants[0]]
        for i in range(len(dict_shift)):
            param = list(dict_shift.keys())[i]
            for j in range(len(dict_shift[param])):
                output = np.concatenate([output,[L_over_parameters_sym_noise(x[0],x[1],x[2],x[3],x[4], par = delta_par[f'{dict_shift[param][j]}{param}'])-\
                    constants[i*(len(dict_shift[param]))+1+j]]])
        return output
    start_LS = time.time()
    roots = least_squares(func_to_zero,x0 = [2.3e-6,2.3e-6,2.3e-6,2.3e-6,0],bounds = ([1.5e-6,1.5e-6,1.5e-6,1.5e-6,-3.2e-6],[3e-6,3e-6,3e-6,3e-6,3.2e-6]),max_nfev = nfev)
    end_LS = time.time()
    time_LS = end_LS-start_LS
    print(time_LS)
    print(roots.nfev)
    ...
    print(roots.fun)
    start = time.time()
    func_to_zero([epsx1,epsy1,epsx2,epsy2,noise_px1])
    end = time.time()
    print(end-start)
    print( LA.norm((np.array(roots.x[:4])-np.array([epsx1,epsy1,epsx2,epsy2]))/LA.norm(np.array([epsx1,epsy1,epsx2,epsy2]))))
...
#         [sol_LS[i+1],f_sol[i+1],Jac_sol[i+1],timing_LS,nfev_LS] = inv_gauss_12(eps1,eps2,dict_shift,verbose=2)
#     LS.append([timing_LS,nfev_LS])
# rel_err = {'est_eps': sol_LS, 'eps': eps, 'f_sol': f_sol, 'Jacobian': Jac_sol, 'LS': LS}
...
#             output = np.concatenate([output,[L_over_parameters_sym(x[0],x[0],x[1],x[1], par = param[f'par{ax[j]}'][keys[i]])-\
#                 constants[(i+1)+j]]])
#     return output 
...
#     # penalty+= (L_over_eps_12(xy[0],xy[1],parameters_sym_2)-constants[1])**2
#     keys = list(paramx['parx'].keys())
#     ax = ['x','y']
#     for i in range(int(len(constants))-1):
#        for j in range(1):
